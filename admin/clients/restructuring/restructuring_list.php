		...
		<div class="card-body">
			...
			<div class="table-responsive">
                <table class="table table-bordered table-stripped" style="text-align:center;font-size:14px;">
                    ...
                    <?php 
                        $i = 1;
                           // $qry = $conn->query("SELECT * FROM tbl_restructuring ORDER BY pay_date DESC") ;


                           $qry = $conn->query("SELECT 
						   z.last_name, z.first_name, z.middle_name,
						   y.property_id, y.payment_amount, y.pay_date, y.due_date, y.or_no,
						   y.amount_due, y.rebate, y.surcharge, y.interest, y.principal,
						   y.remaining_balance, y.payment_count,
						   x.res_id, x.lvl3, x.property_id, x.lvl1, x.lvl2
						 FROM
						   tbl_restructuring x
						 INNER JOIN
						   property_payments y ON y.property_id = x.property_id
						 INNER JOIN
						   property_clients z ON z.property_id = y.property_id
						--  WHERE
						--    x.lvl3 = 0
						GROUP BY
    						y.property_id;
