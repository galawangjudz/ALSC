			...
			<table class="table table-hover table-striped table-bordered" id="sup-div">
				...
				<tbody>
					...
					<tr>
						...
						<td class="text-center">
						<?php $qry_get_pending = $conn->query("SELECT c_status,c_num FROM cv_entries WHERE c_num = '" . $row['c_num'] . "' and c_status = 1 and c_status2 = 0"); ?>
				...
				<tbody>
					...
					<tr>
						...
						<td class="text-center">
							...
							<div class="dropdown-menu" role="menu">
								<?php if ($qry_get_pending->num_rows > 0): ?>
									<button type="button" class="dropdown-item approved_data custom-badge" data-id="<?php echo $row['c_num'] ?>">
										<span class="fa fa-thumbs-up text-success"></span> Approve
									</button>
									<button type="button" class="dropdown-item disapproved_data custom-badge" data-id="<?php echo $row['c_num'] ?>">
										<span class="fa fa-thumbs-down text-danger"></span> Disapprove
									</button>
									<div class="dropdown-divider"></div>
								<?php endif; ?>
...
<script>
	$('.approved_data').click(function(){
		_conf("Are you sure you want to approve this voucher setup?","approved_cv_cfo",[$(this).attr('data-id')])
	})
	$('.disapproved_data').click(function(){
		_conf("Are you sure you want to disapprove this voucher setup?","disapproved_cv_cfo",[$(this).attr('data-id')])
	})
	...
	function approved_cv_cfo($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=approved_cv_cfo",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
	...
	function disapproved_cv_cfo($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=disapproved_cv_cfo",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
