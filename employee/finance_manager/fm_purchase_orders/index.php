...
<div class="card-outline card-primary">
	<div class="card-header">
		...
		<div class="card-tools">
			<a href="./?page=fm_purchase_orders/manage_po" class="btn btn-flat btn-primary" style="font-size:14px;"><span class="fas fa-plus"></span>&nbsp;&nbsp;Create New Purchase Order</a>
		</div>
		<?php endif; ?>
	</div>
	<div class="card-body">
							...
							<tr>
								...
								<td align="center">
									...
									<div class="dropdown-menu" role="menu">
												...
												<a class="dropdown-item" href="?page=fm_purchase_orders/verify_po&id=<?php echo $row['id'] ?>"><span class="fa fa-edit text-primary"></span> Edit</a>
						...
						<tbody>
							...
							<tr>
								...
								<td align="center">
									...
									<div class="dropdown-menu" role="menu">
										<a class="dropdown-item" href="?page=fm_purchase_orders/view_po&id=<?php echo $row['id'] ?>"><span class="fa fa-eye text-primary"></span> View</a>
									</div>
								</td>
							</tr>
							<?php endwhile; ?>
				...
				<table class="table table-bordered table-striped" id="data-table" style="text-align: center; width: 100%; min-width: 1000px;">
					...
					<thead>
						...
						<tbody>
							<?php 
							$i = 1;
								$qry = $conn->query("SELECT po.*, s.name as sname FROM `po_list` po inner join `supplier_list` s on po.supplier_id = s.id WHERE po.status2=3 order by po.date_created DESC");
								while($row = $qry->fetch_assoc()):
									$row['item_count'] = $conn->query("SELECT * FROM order_items where po_id = '{$row['id']}'")->num_rows;
									$row['total_amount'] = $conn->query("SELECT sum(quantity * unit_price) as total FROM order_items where po_id = '{$row['id']}'")->fetch_array()['total'];
...
<script>
	$(document).ready(function(){
		...
		$('.view_details').click(function(){
			uni_modal("Reservaton Details","fm_purchase_orders/view_details.php?id="+$(this).attr('data-id'),'mid-large')
		})
