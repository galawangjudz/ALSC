			...
			<table class="table table-bordered table-stripped" id="data-table" style="text-align:center;width:100%;">
				...
				<?php 
					...
					if ($_settings->userdata('division') == 'MNGR' || $_settings->userdata('division') == 'SPVR') {
						...
						tbl_tba.status1 AS U1,tbl_tba.status2 AS U2, tbl_tba.status3 AS U3, tbl_tba.status4 AS U4, tbl_tba.status5 AS U5,tbl_tba.status6 AS U6,tbl_tba.status7 AS U7 FROM tbl_tba WHERE req_dept = 'Accounting' ORDER BY tbl_tba.transaction_date DESC");
					} else {
										...
										<?php 
											//if ($type > 4) {  
												$qry_filtered = $conn->query("SELECT a.tba_no, a.status1, b.status1, b.status2, b.status3, b.status4, b.status5,
																				...
																				a.tba_no = b.tba_no
																				WHERE a.status1 = 0 AND (b.status1 != '{$a}') AND a.tba_no = '{$tblId}'");
