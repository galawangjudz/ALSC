                            ...
                            <?php
                                ...
                                if ($result) {
                                    ...
                                    <?php
                                        ...
                                        <hr>
                                        <b>GR ID: <i><?= $grId ?></i></b> 
                                        <table id="account_list_<?= $grId ?>" class="table-responsive-sm table-striped table-bordered">
                                                    <colgroup>
                                                        <col width="5%">
                                                        <col width="10%">
                                                        <col width="10%">
                                                    ...
                                                    <tfoot data-gr_id="<?= $grId ?>">
                                                        <tr>
                                                            ...
                                                            <th class="text-right total_debit2">0.00</th>
                                                            <th class="text-right total_credit2">0.00</th>
                                                        </tr>
                                                        <tr>
                                                            <th colspan="4" class="text-center"></th>
                                                            <th colspan="2" class="text-center total-balance2">0</th>
                                                        </tr>
                                                    </tfoot>
                                                </table>
...
function cal_tb() {
    ...
    $('#account_list tbody tr, #account_list tfoot tr').each(function () {
...
<script>
    ...
    function calculateFooter(grId) {
        var totalDebit = 0;
        var totalCredit = 0;

        $('#account_list_' + grId + ' tbody tr').each(function () {
            var debit = parseFloat($(this).find('.debit_amount2').text().replace(/[^0-9.-]/g, '')) || 0;
            var credit = parseFloat($(this).find('.credit_amount2').text().replace(/[^0-9.-]/g, '')) || 0;

            totalDebit += debit;
            totalCredit += credit;
        });

        $('#account_list_' + grId + ' tfoot .total_debit2').text(formatNumberWithCommas(totalDebit));
        $('#account_list_' + grId + ' tfoot .total_credit2').text(formatNumberWithCommas(totalCredit));

        var balance = totalDebit - totalCredit;
        $('#account_list_' + grId + ' tfoot .total-balance2').text(formatNumberWithCommas(balance));
    }

    <?php
    foreach ($groupedData as $grId => $group) {
        echo "calculateFooter($grId);";
    }
    ?>

    function calculateOverallFooter() {
        var totalDebit = 0;
        var totalCredit = 0;

        $('#account_list tbody tr').each(function () {
            var debit = parseFloat($(this).find('.debit_amount').text().replace(/[^0-9.-]/g, '')) || 0;
            var credit = parseFloat($(this).find('.credit_amount').text().replace(/[^0-9.-]/g, '')) || 0;

            totalDebit += debit;
            totalCredit += credit;
        });

        $('#account_list tfoot .total_debit').text(formatNumberWithCommas(totalDebit));
        $('#account_list tfoot .total_credit').text(formatNumberWithCommas(totalCredit));

        var overallBalance = totalDebit - totalCredit;
        $('#account_list tfoot .total-balance').text(formatNumberWithCommas(overallBalance));
    }
    calculateOverallFooter();
</script>
