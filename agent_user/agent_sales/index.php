	...
	<div class="navbar-menu" style="width: 100%; display: flex; justify-content: center; max-width: 1200px; margin: 0 auto;">
		<div class="dropdown">
			...
			<ul class="dropdown-menu" style="border-radius: 0px; height: 122px; margin: 2px; padding-top:2px;"> 
				<li><a href="<?php echo base_url ?>agent_user/?page=agent_sales" style="margin-top: -8px;background-color:#E8E8E8;"><i class="nav-icon fas fa-clock"></i>&nbsp;&nbsp;&nbsp;Pendings</a></li>
					...
					<?php 
						...
						if ($type < 5 ){
							$qry = $conn->query("select q.c_acronym, z.c_block, z.c_lot, y.last_name, y.first_name, y.middle_name, y.suffix_name , x.* from t_csr x , t_csr_buyers y ,
											...
											and y.c_buyer_count = 1 and x.coo_approval=0 and x.c_revised = 0 and x.c_active=1 order by c_date_updated DESC"); ///////REMOVED c_revised
						}else{
							$qry = $conn->query("select q.c_acronym, z.c_block, z.c_lot, y.last_name, y.first_name, y.middle_name, y.suffix_name , x.* from t_csr x , t_csr_buyers y ,
											t_lots z,  t_projects q
											where x.c_csr_no = y.c_csr_no 
											and x.c_lot_lid = z.c_lid 
											and z.c_site = q.c_code 
											and y.c_buyer_count = 1 and ".$where." and x.coo_approval=0 and x.c_revised = 0 and x.c_active=1 order by c_date_updated DESC");  ///////REMOVED c_revised
						}
					...
					?>
						<tr>
							...
							<td align="center">
								...
								<?php if ($row['c_verify'] == 1 && $row['coo_approval'] == 1): ?>
									<a class="dropdown-item view_data" href="./?page=agent_ra/ra-view&id=<?php echo md5($row['c_csr_no']) ?>"><span class="fa fa-eye text-primary"></span> View</a>
								<?php else: ?>
									<a class="dropdown-item view_data" href="./?page=agent_sales/view&id=<?php echo md5($row['c_csr_no']) ?>"><span class="fa fa-eye text-primary"></span> View</a>
								<?php endif; ?>
								<?php if ($row['c_verify'] == 0): ?>
								<div class="dropdown-divider"></div>
								<a class="dropdown-item edit_data" href="./?page=agent_sales/create&id=<?php echo md5($row['c_csr_no']) ?>"><span class="fa fa-edit text-primary"></span> Edit</a>
								<div class="dropdown-divider"></div>
